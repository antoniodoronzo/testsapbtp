...
module.exports = cds.service.impl(async function () {
  ...
  this.after("READ", Risks, async (data) => {
    ...
    const ids = risks.map((x) => x.bp_BusinessPartner);

    const bp = await BPsrv.transaction(this).send({
      query: SELECT.from(this.entities.BusinessPartners)
    ...
    risks.forEach((risk) => {
      ...
      risk.bp = bp.filter(
        (x) => risk.bp_BusinessPartner === x.BusinessPartner
      )[0];
    });
  });
